...
export default {
      ...
      const slots = {
        default: (data) => {
          ...
          if (config.type === 'expand') {
            return this.$slots.expand({ row, column, $index });
          }
          return this.$slots[config.prop]
            ? this.$slots[config.prop]({ row, column, $index })
            : this.generateValue(row, column, config)
        },
        header: ({ column, $index }) => {
          return config.header
            ? this.$slots[`${config.prop}-header`]
  ...
  methods: {
    ...
    generateValue(row, column, config) {
      ...
      return config.editable ? <cell-edit modelValue={res} unit={(config.unit || '')} on-save={(cell) => { this.$parent.$emit('save', parseData(cell)) }} /> : resStr;
    },
